...
<div class="coupon-form-container">
    ...
    <div class="usage-info">
        📊 คูปองนี้ถูกใช้ไปแล้ว <strong>{{ $coupon->used_count }}</strong> ครั้ง
    ...
    <form action="{{ route('coupons.update', $coupon->id) }}" method="POST" class="form-card">
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="number" name="discount_value" class="form-control" step="0.01" min="0" value="{{ old('discount_value', $coupon->discount_value) }}" required>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" name="min_order_amount" class="form-control" step="0.01" min="0" placeholder="ไม่มี" value="{{ old('min_order_amount', $coupon->min_order_amount) }}">
            </div>
            <div class="form-group">
                <label>จำกัดส่วนลดสูงสุด (บาท)</label>
                <input type="number" name="max_discount_amount" class="form-control" step="0.01" min="0" placeholder="ไม่จำกัด" value="{{ old('max_discount_amount', $coupon->max_discount_amount) }}">
            </div>
        </div>

        <div class="form-group">
            <label>จำกัดจำนวนการใช้ (ครั้ง)</label>
            <input type="number" name="usage_limit" class="form-control" min="1" placeholder="ไม่จำกัด" value="{{ old('usage_limit', $coupon->usage_limit) }}">
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="date" name="expire_date" class="form-control" value="{{ old('expire_date', $coupon->expire_date?->format('Y-m-d')) }}">
        ...
        <div class="form-group" style="margin-top: 10px;">
            <label class="switch-container">
                <input type="checkbox" name="status" value="1" {{ old('status', $coupon->status) ? 'checked' : '' }} style="width: 18px; height: 18px; accent-color: #FF8C42; cursor: pointer;">
